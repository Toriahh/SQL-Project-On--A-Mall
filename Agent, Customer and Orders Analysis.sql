...
----To return all customers that whose city are in New York or MUMBAI OR SAN JOSE
SELECT * FROM CUSTOMER
WHERE CUST_CITY = 'New York' OR CUST_CITY = 'Mumbai' OR CUST_CITY = 'San Jose'


---our markerting team want to target customer who lives in the US or Canada for a marketing promotion.
---Return all customers that lives in this area
SELECT * FROM CUSTOMER
WHERE CUST_COUNTRY ='USA' OR CUST_COUNTRY = 'Canada'

---The advertisement team want to create a reward programme and thus targets customers who has an opening amount of above 5000 naira.
---Return all customers in ths category


---The management is asking to know which country the most important customer resides
-- return  a table that shows customer name, customer country, and opening amount in descending order.
...
-- return all neccessary information needed to know the city to be wary of
SELECT CUST_NAME, CUST_CITY, CUST_COUNTRY, OUTSTANDING_AMT FROM CUSTOMER
ORDER BY OUTSTANDING_AMT DESC


---Show all customers that lives in New York
SELECT * FROM CUSTOMER
...
SELECT SUM(RECEIVE_AMT + PAYMENT_AMT) AS sum_of_receive_and_payment_amt FROM CUSTOMER


---Query to find the top customer usinf the TOP filter
SELECT TOP(3) CUST_NAME, CUST_COUNTRY, PAYMENT_AMT FROM CUSTOMER
ORDER BY PAYMENT_AMT DESC


---Info on the city that makes payment the most
...
SELECT TOP(1) CUST_CITY, AVG(OPENING_AMT) AS Average_opening_amount FROM CUSTOMER
GROUP BY CUST_CITY
ORDER BY Average_opening_amount DESC


-- Info on the city with that receives payment the most
SELECT top(1)  CUST_CITY, SUM(RECEIVE_AMT) AS sum_of_received_amount FROM CUSTOMER
GROUP BY CUST_CITY
...
SELECT WORKING_AREA, COUNT(WORKING_AREA) AS number_of_customer FROM CUSTOMER
GROUP BY WORKING_AREA
ORDER BY number_of_customer


-- chose what city to avoid due to rising debt
SELECT CUST_CITY, OUTSTANDING_AMT FROM CUSTOMER
ORDER BY OUTSTANDING_AMT DESC


-- WHAT COUNTRY MAKES THE MOST MONEY (sum all amount columns  and deduct by the debt amount)
SELECT top(1) CUST_COUNTRY, SUM((OPENING_AMT + RECEIVE_AMT + PAYMENT_AMT) - (OUTSTANDING_AMT)) AS total_money  FROM CUSTOMER
GROUP BY CUST_COUNTRY
ORDER BY total_money DESC


-- number of customers in this country above
SELECT CUST_COUNTRY, COUNT(CUST_COUNTRY) AS no_of_customer FROM CUSTOMER
GROUP BY CUST_COUNTRY
ORDER BY no_of_customer



----Operations using the where clause
---BETWEEN: this helps to search for values within a range
---LIKE: it is used to search for partterns in a text
---IN; specify multiple possible values for a column

SELECT CUST_NAME, WORKING_AREA FROM CUSTOMER
WHERE OPENING_AMT BETWEEN 5000 and 8000
AND CUST_NAME LIKE 'Bolt'
AND WORKING_AREA IN ('New York')


----HAVING CLAUSE
SELECT CUST_NAME, SUM(OPENING_AMT) FROM CUSTOMER
GROUP BY CUST_NAME
...
SELECT * FROM CUSTOMER FULL OUTER JOIN ORDERS ON CUSTOMER.CUST_CODE = ORDERS.CUST_CODE
...
--what agent brought the customer that has the heighest opening amount?
WITH VICTORIA AS(
	SELECT CUSTOMER.CUST_NAME, AGENTS.AGENT_NAME, CUSTOMER.OPENING_AMT FROM CUSTOMER
	LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
)
SELECT TOP(1) * FROM VICTORIA
ORDER BY OPENING_AMT DESC


--Show the country of the customer that brought the heighest opening amount
WITH VICTORIA AS(
...
--Show all informations of customers that are from USA
...
WITH VICTORIA AS(
	SELECT CUSTOMER.CUST_NAME, CUSTOMER.PHONE_NO, CUSTOMER.RECEIVE_AMT, AGENTS.AGENT_NAME FROM CUSTOMER
	LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
)
SELECT TOP(1) * FROM VICTORIA
...
WITH VICTORIA AS(
	SELECT ORDERS.ORD_NUM, ORDERS.ORD_DATE, ORDERS.ORD_DESCRIPTION, CUSTOMER.CUST_NAME, AGENTS.AGENT_NAME FROM ORDERS
...
---IDENTIFY THE COLUMN INVOLVED
---IDENTIFY THE TABLES TO FIND THHESE COLUMNS
---JOIN THE TABLES INVOLVED AND SAVE AS NEW NAMES
---QUERY THE NEW TABLE TO FIND YPUR ANSWER
...
--Show the country of the customer that has the heighest order amount
SELECT CUSTOMER.CUST_NAME, CUSTOMER.CUST_COUNTRY, ORDERS.ORD_AMOUNT FROM CUSTOMER
...
--Show the phone number of the agent that brought the customer with the highest received amount
SELECT AGENTS.AGENT_NAME, AGENTS.PHONE_NO,CUSTOMER.CUST_NAME, CUSTOMER.RECEIVE_AMT FROM AGENTS
RIGHT JOIN CUSTOMER ON CUSTOMER.AGENT_CODE = AGENTS.AGENT_CODE
ORDER BY RECEIVE_AMT DESC

--What is the order numbers all sales made on 2008-08-01, and show the information of the agent and customer that is involved in this sales
SELECT ORDERS.ORD_NUM, ORDERS.ORD_DATE, AGENTS.AGENT_NAME, AGENTS.WORKING_AREA, CUSTOMER.CUST_NAME, CUSTOMER.CUST_CODE FROM CUSTOMER
LEFT JOIN ORDERS ON CUSTOMER.CUST_CODE = ORDERS.CUST_CODE
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
WHERE ORD_DATE = '2008-08-01'
...
SELECT AGENTS.*, CUSTOMER.CUST_NAME, CUST_COUNTRY FROM CUSTOMER
RIGHT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
WHERE CUST_COUNTRY = 'USA'

-- Who is our best customer (Total of all amounts less outstanding), what date did he purchase, and who is their agent?
SELECT CUSTOMER.CUST_NAME,  SUM(CUSTOMER.OPENING_AMT + CUSTOMER.RECEIVE_AMT + CUSTOMER.PAYMENT_AMT - CUSTOMER.OUTSTANDING_AMT) AS TOTAL_AMT, ORDERS.ORD_DATE, AGENTS.AGENT_NAME FROM CUSTOMER
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
GROUP BY CUST_NAME, ORD_DATE, AGENT_NAME
...
SELECT CUSTOMER.CUST_NAME, ORDERS.ORD_DATE, AGENTS.AGENT_NAME, AGENTS.PHONE_NO FROM CUSTOMER
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
WHERE CUST_NAME = 'Micheal' AND CUST_CITY = 'Torento'
